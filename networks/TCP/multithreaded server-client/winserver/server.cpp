...
int readNew(PEER *peer, char *data, __int16 buf_len) {
    ...
    while(length < buf_len) {
        bytes = recv(peer->socket, buf, buf_len - length, 0);
        if(bytes > 0) {
            for(data_t i = length; i < length + bytes; i++) {
                data[i] = buf[i-length];
            }
            length += bytes;
            buf[bytes] = '\0';
            cout << +peer->id << " << " << bytes << " " << buf << endl; 
        } else if(bytes == 0) {
            cout << "ID=" << +peer->id << " has closed the connection" << endl; 
            return 0;
        } else {
...
void sendNew(PEER *peer, char *data, data_t buf_len) {
    data_t status = 0;
    data_t bytes = buf_len;
    char *buf= new char[buf_len+1] ;

    do {
        status = send(peer->socket, data, bytes, 0);
        if(status > 0) {
